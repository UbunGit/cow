...
class TransactionListCell: UITableViewCell {
    ...
    var code:String? = nil
    var state:TradeStatus = .didbuy
    private var datas:[TransactionItem] =  []
    ...
    override func updateUI()  {
        ...
        if let hight = Transaction.min(datas){
            let earnings = hight.earnings
            let yield = hight.yield
	...
	func barset(datas:[TransactionItem]) -> BarChartData?  {
        ...
        let targetSets =  datas.enumerated().map { (index,item) -> BarChartDataEntry in
            let set = BarChartDataEntry(x:index.double() , y: item.target)
            return set
        }
